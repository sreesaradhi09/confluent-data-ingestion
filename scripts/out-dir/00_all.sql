...
SELECT
  CAST(JSON_VALUE(CAST(val AS STRING), '$.ADR_ID') AS INT) AS ADR_ID,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_ID') AS INT) AS CI_ID,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CITY') AS STRING) AS CITY,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_ADR_CAN_DT') AS DATE) AS CI_ADR_CAN_DT,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_ADR_EFF_DT') AS DATE) AS CI_ADR_EFF_DT,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_ADR_QLFR_C') AS STRING) AS CI_ADR_QLFR_C,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_AL_1') AS STRING) AS CI_AL_1,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_AL_2') AS STRING) AS CI_AL_2,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_AL_3') AS STRING) AS CI_AL_3,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_AL_4') AS STRING) AS CI_AL_4,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_CNTRY_C') AS STRING) AS CI_CNTRY_C,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_CNTY_C') AS STRING) AS CI_CNTY_C,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_CY') AS STRING) AS CI_CY,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_ST') AS STRING) AS CI_ST,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_Z_C') AS STRING) AS CI_Z_C,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.COUNTRY') AS STRING) AS COUNTRY,
...
SELECT
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CI_ID') AS INT) AS CI_ID,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CLI_ID') AS STRING) AS CLI_ID,
  CAST(JSON_VALUE(CAST(val AS STRING), '$.CNTR_ID') AS STRING) AS CNTR_ID
...
WITH (
  'value.format'='avro-registry', 'connector'='kafka', 'topic'='demo-topic', 'key.format'='json', 'value.format'='json', 'changelog.mode'='upsert'
);
...
WITH (
  'value.format'='avro-registry', 'connector'='kafka', 'topic'='demo-topic', 'key.format'='json', 'value.format'='json'
);
...
-- >>> FGAC_CBA_CI_ADR_REF
INSERT INTO FGAC_CBA_CI_ADR_REF (ADR_ID, CI_ID, CITY, CI_ADR_CAN_DT, CI_ADR_EFF_DT, CI_ADR_QLFR_C, CI_AL_1, CI_AL_2, CI_AL_3, CI_AL_4, CI_CNTRY_C, CI_CNTY_C, CI_CY, CI_ST, CI_Z_C, EMAIL_MASKED, LAST_MTN_DT, LAST_MTN_ID, LAST_MTN_TM, LINE1, REGION, STATE, SYS_STUS_C, UPDT_SEQ_N, ZIP)
SELECT
  a.ADR_ID AS ADR_ID,
  p.CI_ID AS CI_ID,
  a.CITY AS CITY,
  ...
  a.CI_ST AS CI_ST,
  a.CI_Z_C AS CI_Z_C,
  CASE WHEN entitlement.can_view_pii THEN p.CLI_ID ELSE '***' END AS EMAIL_MASKED,
...
FROM `CBA_CI_ADR_view` a
  LEFT JOIN XREF_CBA_CI p ON a.CI_ID = p.CI_ID;
